from pipelines import StreamWan22ICInferencePipeline
...
pipe = StreamWan22ICInferencePipeline(config)
...
for idx in range(len(meta_data)):
    try:
        ...
        export_to_video(video, os.path.join(args.output_path, f'output_{idx}.mp4'), fps=24)
